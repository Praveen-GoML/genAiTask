...
curl -X POST http://localhost:8000/generate \
  ...
  -d '{"prompt": "How do I pair my NovaBuds Pro with my phone?"}'
```
...
```bash
cd ..   # repo root
docker build -f llmops/Dockerfile -t novatech-support-api .
docker run -p 8000:8000 novatech-support-api
```
