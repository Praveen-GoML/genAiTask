...
on the NovaTech electronics customer & tech support dataset in `../data/sample_train.jsonl`, using
...
| `inference_compare.py` | Loads both the base model and the fine-tuned model, runs both on the 20 held-out prompts in `../data/eval_prompts.jsonl`, prints them side by side, and scores each with ROUGE-L. |
