# NovaTech Tech Support — Fine-Tuning, Distillation & LLMOps

A self-contained project for the "Fine-Tuning, Distillation & LLMOps" training pipeline. It demonstrates the **full end-to-end LLM lifecycle**: fine-tuning a base open model with QLoRA, distilling knowledge into a faster student model, and serving/evaluating the assistant via an LLMOps API container.

The task is a **NovaTech Electronics Customer & Tech Support Assistant ("Nova")** — fine-tuned to answer customer inquiries accurately regarding device troubleshooting, warranties, order tracking, returns, and specifications.

```
GenAI-LLMOps/
├── data/
│   ├── sample_train.jsonl        # 150 instruction examples (JSONL chat format)
│   └── eval_prompts.jsonl        # 20 held-out evaluation prompts
