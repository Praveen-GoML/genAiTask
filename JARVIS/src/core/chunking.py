...
class IngestionEngine:
    @staticmethod
    def chunk_txt_sentence(text: str) -> list[str]:
        # Keeps short quips, diagnostic lines, and personality traits distinct
        return [s.strip() for s in text.split('.') if s.strip()]

    @staticmethod
    def chunk_markdown_headers(text: str) -> list[str]:
        # Splits files at the main structural '##' markers
        chunks = []
        sections = text.split("\n## ")
        for sec in sections:
            if sec.strip():
                chunks.append(f"## {sec.strip()}")
        return chunks

    @staticmethod
    def chunk_json_records(text: str) -> list[str]:
        # One array element / ally profiles = one distinct vector record
    ...
    def chunk_csv_rows(text: str) -> list[str]:
        # Forces values to stay strictly bound to column header descriptions
...
INGESTION_PLAN = [
    (chunk_by_sentence, "friday_humor.txt", "friday_humor"),
