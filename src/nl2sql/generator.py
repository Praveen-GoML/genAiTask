...
using the Groq LLM wrapper (src/core/llm). The candidate is untrusted until it passes
src/nl2sql/guard.py AND runs under the read-only DB role — this module's only job is
to produce a reasonable first attempt.
"""
...
def _extract_sql(raw: str) -> str:
    """Models sometimes wrap output in markdown fences despite instructions — strip them."""
